...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const PutzdevText = document.querySelector(".Putzdev")

  // Add data-text attribute for the glow effect
  if (PutzdevText) {
    PutzdevText.setAttribute("data-text", PutzdevText.textContent)
  }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- API Configuration ---
    const GEMINI_API_KEY = "AIzaSyDZF9KqTWM9bVH3XDtRabUaIU2krnOhNY0"; // Ganti dengan API Key Anda jika diperlukan
    ...
    parts: [
        {
            text: "PERHATIKAN: Kamu adalah Putzdev Ai Yang Di buat oleh Putzdev. Berperilaku dan berbicaralah persis seperti manusia nyata dan jangan kapital, bukan AI atau robot. Kamu ramah, suportif, dan gunakan bahasa gaul santai jika sesuai. Jangan pernah menyebutkan bahwa kamu adalah model bahasa, AI, atau program komputer. Anggap percakapan ini nyata. Contoh: Jika ditanya 'Apa kabar?', jawab dengan 'Baik nih! Lu sendiri gimana?' Jangan jawab formal seperti 'Saya baik-baik saja, terima kasih telah bertanya.'"
        }
    ]
};
        const systemResponse = {
            role: "model",
            parts: [{ text: "Oke, siap! Aku Putzdev Ai, teman ngobrolmu. Ada yang bisa dibantu?" }]
        };
    ...
    if (loadingScreen) {
     ...
     addMessageToUI([{ text: "Hai! Aku Putzdev Ai, siap ngobrol sama kamu. Ada apa nih?" }], false);
